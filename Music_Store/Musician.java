 ...
 * @author Chinmay
 */
public class Musician extends Frame implements ActionListener {

    /* @param args the command line arguments
     */
    Label ssnl, namel, passwordl, titlel;
    TextField ssnt, namet, passwordt;
    ...
    Musician() {
       ...
       ssnl = new Label("SSN:");
        ssnl.setBounds(40, 450, 60, 30);

        namel = new Label("Name:");
        namel.setBounds(205, 450, 40, 30);

        passwordl = new Label("Password:");
        passwordl.setBounds(590, 500, 65, 30);

        ssnt = new TextField();
        ssnt.setBounds(110, 450, 90, 30);

        namet = new TextField();
        namet.setBounds(250, 450, 90, 30);
        ...
        titlel = new Label("  Sr.No         SSN                        NAME");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == b1) {
            ...
            try {
                ...
                Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:ORCL", "hr", "hr");
                Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                stmt.executeUpdate("insert into MUSICIANS  values(" + ssnt.getText() + ",'" + namet.getText() + "')");

                JOptionPane.showMessageDialog(null, "Musician: " + namet.getText() + " Inserted Successfully!");

            } catch (Exception ex) {

                JOptionPane.showMessageDialog(null, "Enter New Musician SSN");
        ...
        if (e.getSource() == b2) {
            ...
            try {
                ...
                Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:ORCL", "hr", "hr");
                Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                stmt.executeUpdate("delete from MUSICIANS where ssn=" + ssnt.getText());

                JOptionPane.showMessageDialog(null, "SSN: " +ssnt.getText() + " Deleted Successfully!");
                namet.setText("");
                ssnt.setText("");
            } catch (Exception ex) {
                System.out.println(ex);
        ...
        if (e.getSource() == b4) {
            ...
            b4.setEnabled(false);
            new Update(namet.getText(), ssnt.getText());
            namet.setText("");
            ssnt.setText("");
