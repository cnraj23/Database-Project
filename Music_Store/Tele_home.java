 ...
 * @author Chinmay
 */
public class Tele_home extends Frame implements ActionListener {

    /* @param args the command line arguments
     */
    Label phonel, addressl, passwordl, titlel;
    TextField phonet, addresst, passwordt;
    ...
    Tele_home() {
        ...
        setVisible(true);
        setLayout(null);
        setTitle("Telephone Home Record");
        setResizable(false);
        ...
        titlel = new Label("  Sr.No      PHONE          ADDRESS");
    ...
    public void display() {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery("select * from TELEPHONE_HOME");
            rs.beforeFirst();
            ta.setText("");
            while (rs.next()) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == b1) {
            ...
            try {
                ...
                Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:ORCL", "hr", "hr");
                Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                stmt.executeUpdate("insert into TELEPHONE_HOME  values(" + phonet.getText() + ",'" + addresst.getText() + "')");

                JOptionPane.showMessageDialog(null, "Telephone: " + phonet.getText() + " Inserted Successfully!");

            } catch (Exception ex) {

                JOptionPane.showMessageDialog(null, "Enter New Telephone!");
                System.out.println(ex);
            }
            phonet.setText("");
            addresst.setText("");
            passwordt.setText("");
            phonet.setFocusable(true);
        ...
        if (e.getSource() == b2) {
            ...
            try {
                ...
                Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:ORCL", "hr", "hr");
                Statement stmt = con.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
                stmt.executeUpdate("delete from TELEPHONE_HOME where phone=" + phonet.getText());

                JOptionPane.showMessageDialog(null, "Phone: " + phonet.getText() + " Deleted Successfully!");
                phonet.setText("");
                addresst.setText("");
            } catch (Exception ex) {
                System.out.println(ex);
        ...
        if (e.getSource() == b4) {
            ...
            b4.setEnabled(false);
            new Update_phone(phonet.getText(), addresst.getText());
            phonet.setText("");
            addresst.setText("");
